    ...
    if(tracklist.length > 0) {
      ...
      for (let i = 0; i < tracklist.length; i++) {
        ...
        if (inList.length > 0) {
          ...
          tracksInPlaylist.push(testTrack);
        } else {
          testTrack.inPlaylist = "false";
          tracksNotInPlaylist.push(testTrack);
        };
      };
      console.log(changesMade);
      if (changesMade) {setTracklist(tracksNotInPlaylist.concat(tracksInPlaylist));}
    }
  },[tracklist, playlist])
    ...
    <div className="App">
      ...
      <main>
        <div className="list-container tracklist-container">
          <Tracklist tracklist={tracklist} onSubmitForm={doSearch} onAddTrack={onAddTrackHandler}/>
        </div>
